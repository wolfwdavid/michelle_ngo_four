...
- [x] **First-class Press page surfacing HBO Max, PBS, Hulu, Amazon, U2 Sphere, and other broadcast credits** — `/press` renders 13 broadcast credits grouped by uploader in locked prestige order (HBO Max → HBO → PBS → ABC News → U2 → Amazon News → Music Box Films → Monument Releasing → Cargo Film & Releasing → AZPM → HBODocs → GrasshalmClips → Lenny Cooke (Movie)); each credit links to `/watch/[id]`; prerender coverage script asserts the output. (Validated in Phase 6: Press, About & Contact — PRES-01, PRES-02)
- [x] **About page with bio, headshot, contact info, IMDb + LinkedIn links** — `/about` renders user-approved first-person bio (~109 words, D-18 punchy band) + embedded `<ContactBlock />` with email/phone/IMDb/LinkedIn/Vimeo. Headshot deferred per planner D-20. IMDb + LinkedIn URLs ship as channel-homepage fallbacks per user direction 2026-05-12; pre-cutover personalized-URL swap tracked in 06-HUMAN-UAT.md. (Validated in Phase 6: Press, About & Contact — ABT-01, ABT-02)
- [x] **Contact surface (email + phone + socials in footer; About page mirrors it)** — Shared `<ContactBlock />` component (`mailto:` literal lives ONLY in ContactBlock.svelte = single source of truth) renders on `/contact`, `/about`, AND inside the site-wide `<Footer />`. Footer wired into `+layout.svelte` so 5 contact channels appear on every prerendered route (verified: `mynogo@gmail.com` literal in 70 prerendered HTML files). Footer also mirrors TopNav (categories + secondary nav). (Validated in Phase 6: Press, About & Contact — CONT-01, CONT-02, NAV-02)
...
- [x] **Production-grade metadata + cutover infrastructure ready** — per-page `<title>` + meta descriptions across 7 routes; sitewide OG/Twitter cards; Person JSON-LD on `/about`; VideoObject JSON-LD on every `/watch/[id]`; build-time `sitemap.xml` endpoint (70 URLs); favicon set + og-image (PLACEHOLDER assets, designed-replacement is post-launch backlog). Cutover infrastructure: `static/CNAME` = `michellengo.net`; `.github/workflows/deploy-production.yml` with `BASE_PATH=''`. Self-contained 9-step Launch Runbook in `07-05-production-cutover-SUMMARY.md` for cutover-day execution. FOUND-03 reconciled to `Accepted-Deferred` (Lighthouse perf gate moved to post-launch real-user telemetry; if LCP > 2.0s reopen + apply D-08 escalation). (Validated in Phase 7: Polish & Production Cutover — FOUND-03)

### Active

_None — all v1 requirements validated. Cutover is a documented human-action runbook (07-05-SUMMARY.md § Launch Runbook), not an open requirement._
...
| Press as first-class section | HBO Max, PBS, Hulu, U2 Sphere, Amazon credits are commercial differentiators for the hiring-producer audience | ✓ Validated in Phase 6 — `/press` ships 13 prestige-ordered broadcast credits, each linking to its `/watch/[id]` |
| **D-deviation: IMDb + LinkedIn URLs ship as channel-homepage fallbacks** | User did not have personalized profile URLs ready at execution time on 2026-05-12 and explicitly chose to proceed rather than block Phase 6. Channel homepages are functional (no 404s), pass domain-contains tests, and are a single-line swap | ⚠ Tracked in 06-HUMAN-UAT.md — pre-michellengo.net-cutover blocker; surfaces in `/gsd:progress` and `/gsd:audit-uat` until resolved |
| ContactBlock as single source of truth for contact channels | One component renders on `/about`, `/contact`, AND the site-wide footer; `mailto:` literal lives in exactly one file | ✓ Validated in Phase 6 — pre-cutover URL swap is a single-file edit |
| **Phase 7 deferral pattern: pragmatic v1.0 launch posture** | Five plans, four ended in `accepted-deferred` (07-01 IMDb URLs, 07-02 placeholder favicon+og-image assets, 07-03 fast-path QA acceptance, 07-04 Lighthouse → post-launch telemetry, 07-05 stop-at-infrastructure-ready) per explicit user decision at each checkpoint. Real perf signal comes from production telemetry, not synthetic Slow-4G simulation; cutover-day human-action steps captured in self-contained Launch Runbook | ✓ Validated in Phase 7 — 0 codebase contradictions per `07-VERIFICATION.md`; FOUND-03 reconciled to `Accepted-Deferred`; 3 items tracked in `07-HUMAN-UAT.md` for cutover-day execution |
...
- **Phase 6 (Press, About & Contact): Complete** — three content pages + shared contact infrastructure shipped. `/press` ships 13 prestige-ordered broadcast credits derived from videos.json with per-credit `/watch/[id]` links. `/about` ships user-approved first-person bio (~109 words) + embedded `<ContactBlock />`. `/contact` reuses the same `<ContactBlock />`. Site-wide `<Footer />` (3-column desktop / 1-column mobile / copyright strip) is wired into `+layout.svelte`, mirroring TopNav and surfacing the 5 contact channels on every prerendered route (`mynogo@gmail.com` literal verified in 70 prerendered HTML files). 168/168 unit tests passing across 20 files (29 new in this phase). All 7 requirement IDs satisfied (PRES-01, PRES-02, ABT-01, ABT-02, CONT-01, CONT-02, NAV-02). **Tracked deviation:** IMDb + LinkedIn URLs ship as channel-homepage fallbacks per user direction 2026-05-12; pre-michellengo.net-cutover personalized-URL swap persisted in `06-HUMAN-UAT.md` so it surfaces in `/gsd:progress` and `/gsd:audit-uat` until resolved
- **Phase 7 (Polish & Production Cutover): Complete (human-needed for cutover)** — production-grade metadata wired across all 7 routes (per-page titles + descriptions, OG/Twitter cards, Person JSON-LD on `/about`, VideoObject JSON-LD on every `/watch/[id]`, build-time `sitemap.xml` with 70 URLs, placeholder favicon set + og-image). Cutover infrastructure shipped: `static/CNAME` = `michellengo.net`; `.github/workflows/deploy-production.yml` with `BASE_PATH=''`; staging deploy workflow unchanged. 168/168 tests still passing after metadata wiring. FOUND-03 reconciled to `Accepted-Deferred` (Lighthouse perf gate deferred to post-launch real-user telemetry; 15.4KB hero WebP + preload + fetchpriority=high + loading=eager budget intact). **Cutover-day path:** execute the self-contained 9-step Launch Runbook in `07-05-production-cutover-SUMMARY.md` § Launch Runbook (push commits → trigger prod workflow → add custom domain → cert wait 15-60min → curl --resolve verify → D-16 atomic noindex+robots flip → re-trigger prod workflow → DNS swap at registrar → post-cutover verify; rollback documented). **Post-launch backlog:** author proper favicons + 1200×630 OG image (drop into `static/`, no source edit); swap IMDb/LinkedIn URLs in ContactBlock + Person JSON-LD when personalized URLs land; monitor production LCP via real-user telemetry; reopen FOUND-03 + apply D-08 escalation if > 2.0s
- Next: Execute Launch Runbook on cutover day; close milestone v1.0 with `/gsd:complete-milestone` after cutover

---
*Last updated: 2026-05-19 after Phase 7 completion*
