...
- [x] **FOUND-03**: User sees the production build under 2s on a 4G connection (no client-side data fetch on first paint) — *Accepted-Deferred at v1.0 (2026-05-16 via Plan 07-04): synthetic Lighthouse Slow-4G audit skipped; budget intact at source (Phase 4 D-04..D-07: 15.4KB WebP hero + preload + fetchpriority=high + loading=eager); post-launch real-user telemetry on `michellengo.net` is the measured signal. Reopen + apply D-08 escalation if real-user LCP > 2.0s on /.*
...
- [x] **NAV-02**: Footer mirrors the top nav for accessibility and discoverability
...
| FOUND-03 | Phase 7 | Accepted-Deferred |
